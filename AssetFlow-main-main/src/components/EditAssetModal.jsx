...
const EditAssetModal = ({ isOpen, onClose, onSuccess, asset }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.error || err.message || 'Failed to edit asset');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      <div className="card" style={{ width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
        ...
        <h2 style={{ marginBottom: '1.5rem', fontSize: '1.25rem' }}>Edit Asset</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <select className="form-input" name="categoryId" value={formData.categoryId} onChange={handleChange}>
          ...
          <div className="form-group" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem', marginTop: '0.5rem' }}>
            <input type="checkbox" id="isBookableEdit" name="isBookable" checked={formData.isBookable} onChange={handleChange} style={{ width: '1.125rem', height: '1.125rem' }} />
            <label htmlFor="isBookableEdit" style={{ margin: 0, cursor: 'pointer', fontSize: '0.875rem' }}>Is this asset bookable by employees?</label>
          </div>
